function lab_04
    ...
    % Задать сигнал Гаусса с на промежутке t с параметром sigma
    x0 = gauspls(t, sigma);     % функция описана ниже

    % Получаем помеху Гаусса и прибавляем ее к основному сигналу
    NA = 0;
    NS = 0.05;
    n1 = normrnd(NA,NS,[1 length(x0)]);
    x1 = x0+n1;

    % Получаем импульсную помеху и прибавляем ее к сигналу Гаусса
    count = 7;
    M = 0.4;
    n2 = impnoise(length(x0),count,M);  % функция описана ниже
    x2 = x0+n2;

    % Получили фильтры (Гаусса и Баттеруорта)
    [B,A] = butter(6,0.05,'low');
    G = gaussfilt(4,20,'low');      % наша функция, описана ниже
    BB = buttfilt(6,20,'low');      % наша функция, описана ниже
